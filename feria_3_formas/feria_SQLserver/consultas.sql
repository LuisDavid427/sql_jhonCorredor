...
select * from feria as f
inner join pabellon as p on f.id_feria = p.id_feria;

select * from tematica as t
...
create view visitantes_con_tipo
as
...
create view registros_por_feria_y_empresa
as
...
create view total_stands_por_feria
as
select f.nombre as feria, count(s.id_stand) as total_stands
from feria f
inner join pabellon p on f.id_feria = p.id_feria
...
create view responsables_de_productos_por_empresa
as
select per.nombre as responsable, pr.nombre_producto, e.nombre_empresa
from responsable r
inner join persona per on r.id_persona = per.id_persona
