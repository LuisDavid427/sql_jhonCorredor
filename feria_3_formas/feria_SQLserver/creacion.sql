...
create table log (
    ...
    datos_old nvarchar(max),
    datos_new nvarchar(max)
);

create table feria(
    id_feria int primary key identity(1,1),
    nombre varchar(60),
    descripcion nvarchar(max)
);

create table tematica(
    id_tematica int primary key identity(1,1),
    tematica varchar(20)
);

create table pabellon(
    id_pabellon int primary key identity(1,1),
    nombre varchar (60),
    id_tematica int,
    id_feria int,
...
create table ponente(
    id_ponente int primary key identity(1,1),
    id_persona int,
    foreign key (id_persona) references persona (id_persona)
);
...
create table charla (
    id_charla int primary key identity(1,1),
    nombre_charla varchar (80),
    descripcion nvarchar(max)
);

create table demostracion (
    id_demostracion int primary key identity(1,1),
